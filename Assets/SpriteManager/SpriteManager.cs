...
public class SpriteManager : MonoBehaviour
{
    ...
    protected SMSprite[] sprites;             // Array of all sprites (the offset of the vertices corresponding to each sprite should be found simply by taking the sprite's index * 4 (4 verts per sprite).
    ...
    protected ArrayList spriteDrawOrder = new ArrayList();  // Array of indices of sprite objects stored in the order they are to be drawn (corresponding to the position of their vertex indices in the triIndices list)  Allows us to keep track of wherea given Sprite is in the drawing order (triIndices)
    ...
    // Working vars:
    protected int i;
    protected SMSprite tempSprite = null;
    ...
    public void ModifyColorArrayDirectly(Color[] colors)
    {
        ...
        {
            Debug.LogWarning("Wrong SMManager sizes: " + colors.Length + " vs " + this.colors.Length);
        }
        colorsChanged = true;
    }
    ...
    {
        sprites = new SMSprite[1];
        sprites[0] = new SMSprite();
    ...
    protected int EnlargeArrays(int count)
    {
        ...
        if (sprites == null)
        {
            InitArrays();
            firstNewElement = 0;
            count--;  // Allocate one less since InitArrays already allocated one sprite for us
        }
        else
            firstNewElement = sprites.Length;

        // Resize sprite array:
        SMSprite[] tempSprites = sprites;
        sprites = new SMSprite[sprites.Length + count];
        tempSprites.CopyTo(sprites, 0);

        length = sprites.Length + count;
    ...
    public SMSprite AddSprite(GameObject client, float width, float height, int leftPixelX, int bottomPixelY, int pixelWidth, int pixelHeight, bool billboarded)
    {
    ...
    public SMSprite AddSprite(GameObject client, float width, float height, int leftPixelX, int bottomPixelY, int pixelWidth, int pixelHeight, Vector3 offset, bool billboarded)
    {
    ...
    // offset - the sprite will be offseted by the specified Vector3 relative to the client transform
    public SMSprite AddSprite(GameObject client, float width, float height, Vector2 lowerLeftUV, Vector2 UVDimensions, Vector3 offset, bool billboarded)
    {
        int spriteIndex;
        ...
        Vector3 normal = CalculateNormal(newSprite, spriteIndex * 6);
    ...
    private Vector3 CalculateNormal(SMSprite sprite, int offset)
    {
    ...
    private Vector3 CalculateNormal(SMSprite sprite)
    {
    ...
    public void SetBillboarded(SMSprite sprite)
    {
    ...
    public void RemoveSprite(SMSprite sprite)
    {
    ...
    public void HideSprite(SMSprite sprite)
    {
        // Remove the sprite from the billboarded list
    ...
    public void ShowSprite(SMSprite sprite)
    {
    ...
    public void MoveToFront(SMSprite s)
    {
    ...
    public void MoveToBack(SMSprite s)
    {
        int[] indices = new int[6];
        int offset = spriteDrawOrder.IndexOf(s) * 6;
    ...
    public void MoveInfrontOf(SMSprite toMove, SMSprite reference)
    {
        int[] indices = new int[6];
    ...
    public void MoveBehind(SMSprite toMove, SMSprite reference)
    {
        int[] indices = new int[6];
        int offset = spriteDrawOrder.IndexOf(toMove) * 6;
        int refOffset = spriteDrawOrder.IndexOf(reference) * 6;
    ...
    public SMSprite GetSprite(int i)
    {
        if (i < sprites.Length)
            return sprites[i];
        else
    ...
    public void Transform(SMSprite sprite)
    {
        sprite.Transform();
    ...
    public void TransformBillboarded(SMSprite sprite)
    {
    ...
    public void UpdateUV(SMSprite sprite)
    {
    ...
    public void UpdateColors(SMSprite sprite)
    {
